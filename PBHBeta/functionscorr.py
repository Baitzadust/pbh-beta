...
# ---------------------------------------------------------------------------
# Cuadrícula de interpolación calibrada con BlackHawk (fM.txt)
# ---------------------------------------------------------------------------
...
def kerr_evap_factor(a_star):
    """
    Factor de amplificación de evaporación xi(a*) de Kerr normalizado a Schwarzschild.
    Garantiza continuidad y positividad exacta en todo el dominio [0, 0.9999].
    """
    a = jnp.clip(a_star, 0.0, 0.9999)
    return jnp.interp(a, SPINS_GRID, XI_GRID)

def calcular_masa_critica_analitica(N_fin, a_star, lam, gamma_val):
    """
    Calcula la masa crítica analítica de transición incorporando:
      - xi(a*) de BlackHawk en la tasa de evaporación
      - f_espin(a*) en la ganancia de acreción mu
    """
    a_safe = np.clip(a_star, 0.0, 0.9999)
    
    # 1. Factor de realce de evaporación de Kerr xi(a*)
    xi_a = kerr_evap_factor(a_safe)
    
    # 2. Factor geométrico de captura para acreción
    raiz_espin = np.sqrt(1.0 - a_safe**2)
    f_espin = ((1.0 + raiz_espin) / 2.0)**2
    lam_eff = lam * f_espin
    
    # 3. Factor de ganancia asintótico mu
    denom = 1.0 - 2.0 * lam_eff * gamma_val
    mu = 1.0 / denom if denom > 0.0 else np.inf
    
    # 4. Parámetro de Hubble en N_fin (unidades de Planck)
    H_Nfin = H_end_pl * np.exp(-1.5 * N_fin)
    
    # 5. Coeficiente base de evaporación:
    # Si tu ODE usa: dM/dt = -(1.0 / (3.0 * M^2)) * factor_evap_kerr,
    # entonces alpha_0 = 1.0 / 3.0 en unidades de Planck.
    alpha_0 = 1.0 / 3.0
    
    # Masa crítica en unidades de Planck y conversión a gramos
    M_c_pl = (1.0 / mu) * ((2.0 * alpha_0 * xi_a) / H_Nfin)**(1.0 / 3.0)
    M_c_g = M_c_pl * M_pl_g
...
def precalcular_acreccion_lote(Mi_val_g, N_fin, a_star, lam=2.0, gamma=0.2):
    M_i_pl = Mi_val_g / M_pl_g
    M_end_pl = 1.0 / H_end_pl
    
    # 1. Corrección de formación: PBH nace cuando M_H = M_i / gamma
    ...
    # Amplitud inicial para que la densidad coincida con el fondo cósmico
    rho_end_inf_pl = (3.0 * H_end_pl**2.0) / (8.0 * jnp.pi)
    phi_ini_pl = jnp.sqrt(2.0 * rho_end_inf_pl / (mu_pl**2.0 + 9.0 * H_end_pl**2.0 / 4.0))

    # Factor geométrico de espín para la captura
    raiz_espin = jnp.sqrt(1.0 - a_star_safe**2.0)
    factor_espin_acc = ((1.0 + raiz_espin) / 2.0)**2.0

    # Condición de Runaway (L_eff * gamma < 4*pi  <=> 2 * lam_eff * gamma < 1.0)
    lambda_eff_max = lam * factor_espin_acc
    is_runaway = 2.0 * lambda_eff_max * gamma >= 1.0
    
    # Factor de evaporación de Kerr evaluado para este PBH
    ...
    def vector_field_log(N, u, args):
        ...
        # 1. Tasa de Acreción de Ondas
        lambda_eff = lam * factor_espin_acc * S_onda
        dlnM_dN_acc = 3.0 * lambda_eff * x * Omega_phi
        
        # 2. Evaporación de Hawking-Kerr calibrada con BlackHawk
    ...
    solver = Tsit5()
    # jump_ts para forzar al integrador a no saltar el e-fold exacto de formación
